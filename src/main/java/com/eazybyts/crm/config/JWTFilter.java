	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (!openRoutes.contains(request.getRequestURI())) {
			...
			if (!StringUtils.hasText(bearerToken) || !bearerToken.startsWith("Bearer ")) {
				...
				printWriter
						.print(mapper.writeValueAsString(new ApiResponseDto("Token is missing in the header!", 400,null)));
				printWriter.flush();
				return;
			}
			...
			try {
				...
				PrintWriter printWriter = response.getWriter();
				printWriter.print(
						mapper.writeValueAsString(new ApiResponseDto("Token is either invalid or expired!", 400,null)));
				printWriter.flush();
				return;
			}
			if (Objects.nonNull(userName) && Objects.isNull(SecurityContextHolder.getContext().getAuthentication())) {
				...
				try {
					...
					if (jwtUtility.validateToken(bearerToken, userDetails.getUsername())) {
						...
						printWriter.print(mapper
								.writeValueAsString(new ApiResponseDto("Token is either invalid or expired!", 400,null)));
						printWriter.flush();
						return;
					}
				} catch (Exception e) {
					...
					PrintWriter printWriter = response.getWriter();
					printWriter.print(
							mapper.writeValueAsString(new ApiResponseDto("Unable to authenticate the request!", 500,null)));
					printWriter.flush();
					return;
				}
			} else {
				...
				PrintWriter printWriter = response.getWriter();
				printWriter
						.print(mapper.writeValueAsString(new ApiResponseDto("Unable to validate the request!", 500,null)));
				printWriter.flush();
				return;
			}
		} else {
